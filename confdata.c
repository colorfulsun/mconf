...
static _Bool is_present(const char *path)
{
	struct stat st;
...
static _Bool is_dir(const char *path)
{
	struct stat st;
...
int conf_write_defconfig(const char *filename)
{
	...
	while (menu != NULL)
	{
		...
		if (sym == NULL) {
			 ...
			 * But only if value is _Bool and equal to "y" and
...
int conf_write(const char *name)
{
	...
	const char *basename;
	const char *str;
	char dirname[4096+1], tmpname[4096+22], newname[4096+8];
	char *env;
...
void sym_set_change_count(int count)
{
	...
	if (conf_changed_callback &&
	    (_Bool)_sym_change_count != (_Bool)count)
...
_Bool conf_set_all_new_symbols(enum conf_def_mode mode)
{
	...
	int i, cnt, pby, pty, ptm;	/* pby: probability of _Bool     = y
